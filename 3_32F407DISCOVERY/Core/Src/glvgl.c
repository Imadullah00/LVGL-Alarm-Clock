...
{
    lv_obj_set_style_bg_color(me->screen, lv_palette_main(LV_PALETTE_LIGHT_BLUE), LV_PART_MAIN);
    ...
    /*dropdown btn setup*/
    lv_obj_remove_style_all(me->dropdown_btn);
    lv_obj_set_size(me->dropdown_btn, 30, 30);
    lv_obj_align(me->dropdown_btn, LV_ALIGN_TOP_RIGHT, 0, 0);
    lv_obj_set_style_bg_opa(me->dropdown_btn, LV_OPA_90, LV_STATE_PRESSED);
    lv_obj_set_style_radius(me->dropdown_btn, 5, LV_STATE_PRESSED);
    lv_obj_set_style_bg_color(me->dropdown_btn, lv_palette_main(LV_PALETTE_AMBER), LV_STATE_PRESSED);
    lv_obj_t* label = lv_label_create(me->dropdown_btn);
...
void Lvgl_create_main_page(Lvgl * const me) {
    ...
    me->dropdown_btn = lv_btn_create(me->screen);
   
    lv_obj_add_event_cb(me->screen, event_handler_screen, LV_EVENT_PRESSED, (void*)me);
    lv_obj_add_event_cb(me->dropdown_btn, event_handler_dropdown_btn, LV_EVENT_CLICKED, (void*)me);
...
void Lvgl_create_setting_page(Lvgl * const me, lvgl_setting_page_data_t* data) {
    ...
    /*Send event manually to 'default_cont'*/
